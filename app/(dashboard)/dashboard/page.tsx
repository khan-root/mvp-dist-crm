...
export default async function DashboardPage() {
  ...
  function hasAccess(moduleName: string): boolean {
    if (!currentUser) return true;
    if (currentUser.role_code === "admin") return true;
    if (!currentUser.permissions || !currentUser.permissions.modules) return true;
    const mod = currentUser.permissions.modules.find(
      (m: any) => m.module_name === moduleName || m.module_name === "all"
    );
    return Boolean(mod && mod.actions && mod.actions.length > 0);
  }

  const allCards = [
    { title: "Inbound Cargo & Bilties", value: "Active", href: "/dashboard/supply-chain/bilties", icon: FileSpreadsheet, color: "bg-emerald-50 text-emerald-700", module: "supply_chain" },
    { title: "Fleet & Logistics", value: "Operational", href: "/dashboard/supply-chain/vehicles", icon: Truck, color: "bg-blue-50 text-blue-700", module: "supply_chain" },
    { title: "Distributors", value: counts.distributors, href: "/dashboard/distributors", icon: Building2, color: "bg-slate-100 text-slate-800", module: "distributors" },
    { title: "Field Agents", value: counts.agents, href: "/dashboard/agents", icon: Users, color: "bg-slate-100 text-slate-800", module: "agents" },
    { title: "Stores & Outlets", value: counts.stores, href: "/dashboard/stores", icon: Store, color: "bg-emerald-50 text-emerald-700", module: "stores" },
    { title: "Warehouses", value: counts.warehouses, href: "/dashboard/warehouses", icon: Warehouse, color: "bg-amber-50 text-amber-700", module: "inventory" },
    { title: "Products & SKUs", value: counts.products, href: "/dashboard/products", icon: Package, color: "bg-slate-100 text-slate-800", module: "products" },
    { title: "Total Orders", value: counts.orders, href: "/dashboard/orders", icon: ShoppingCart, color: "bg-emerald-50 text-emerald-700", module: "orders" },
  ];

  const visibleCards = allCards.filter((c) => hasAccess(c.module));

  const canViewOrders = hasAccess("orders");
  const canViewNetwork = hasAccess("distributors") || hasAccess("agents") || hasAccess("stores") || hasAccess("routes");
  const canViewSupplyChain = hasAccess("supply_chain");

  let headerAction = { label: "Manage Orders", href: "/dashboard/orders" };
  if (!canViewOrders) {
    if (canViewSupplyChain) headerAction = { label: "SCM Control Tower", href: "/dashboard/supply-chain" };
    else if (hasAccess("inventory")) headerAction = { label: "Stock Inventory", href: "/dashboard/inventory" };
    else if (hasAccess("routes")) headerAction = { label: "Sales Routes", href: "/dashboard/routes" };
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-slate-900 text-white shadow-md">
        <div>
          ...
          <p className="text-sm text-slate-300">
            Real-time overview of field sales, journey routes, and logistics.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button asChild size="sm" className="bg-emerald-600 hover:bg-emerald-500 text-white shadow-md">
            <Link href={headerAction.href}>
              {headerAction.label} <ArrowRight className="size-4 ml-1.5" />
            </Link>
          </Button>
        </div>
      </div>
      ...
      {/* Orders or Supply Chain Command Section */}
      {canViewOrders ? (
        ...
        <Card className="border-slate-200 shadow-sm bg-slate-900 text-white">
          <CardHeader>
            <div className="flex items-center gap-2">
              <Ship className="size-6 text-emerald-400" />
              <div>
                <CardTitle className="text-xl font-bold">Port & Supply Chain Command Studio</CardTitle>
                <CardDescription className="text-slate-300">
                  Quick access to inbound cargo receipts, transport bilties, and port operations.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              <Link href="/dashboard/supply-chain/shipments" className="p-4 rounded-xl bg-slate-800/80 border border-slate-700 hover:border-emerald-500 transition-all space-y-2 group">
                <div className="flex items-center justify-between text-emerald-400">
                  <Boxes className="size-5" />
                  <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
                </div>
                <div className="font-bold text-sm text-white">Inbound Goods Receipts</div>
                <p className="text-xs text-slate-400">Record cargo arrivals & bilties</p>
              </Link>

              <Link href="/dashboard/supply-chain/bilties" className="p-4 rounded-xl bg-slate-800/80 border border-slate-700 hover:border-emerald-500 transition-all space-y-2 group">
                <div className="flex items-center justify-between text-sky-400">
                  <FileSpreadsheet className="size-5" />
                  <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
                </div>
                <div className="font-bold text-sm text-white">Transport Bilties (TM)</div>
                <p className="text-xs text-slate-400">Track freight bilties & carriers</p>
              </Link>

              <Link href="/dashboard/supply-chain/vehicles" className="p-4 rounded-xl bg-slate-800/80 border border-slate-700 hover:border-emerald-500 transition-all space-y-2 group">
                <div className="flex items-center justify-between text-purple-400">
                  <Truck className="size-5" />
                  <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
                </div>
                <div className="font-bold text-sm text-white">Fleet & Vehicles</div>
                <p className="text-xs text-slate-400">Manage port transport trucks</p>
              </Link>

              <Link href="/dashboard/supply-chain" className="p-4 rounded-xl bg-slate-800/80 border border-slate-700 hover:border-emerald-500 transition-all space-y-2 group">
                <div className="flex items-center justify-between text-amber-400">
                  <Ship className="size-5" />
                  <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
                </div>
                <div className="font-bold text-sm text-white">SCM Control Tower</div>
                <p className="text-xs text-slate-400">Full port movement overview</p>
              </Link>
            </div>
          </CardContent>
        </Card>
